...
inline void RegisterAllTranslations()
{
	...
	ADD_STR("Menu_IsBlur", "Blur menu", u8"Размывать меню", u8"菜单模糊效果");
	ADD_STR("Menu_IsRain", "Rain effect", u8"Эффект дождя", u8"菜单雨滴效果");
    ADD_STR("Menu_SettText", "Menu settings", u8"Настройки меню", u8"菜单设置");
    ADD_STR("Menu_ApplyColors", "Apply colors", u8"Применить цвета", u8"应用配色");
	ADD_STR("Menu_ResetColors", "Reset colors", u8"Сбросить цвета", u8"重置配色");
    ADD_STR("Menu_SetMenuKey", "Set menu opening key", u8"Задать клавишу открытия меню", u8"设置菜单快捷键");
	ADD_STR("Proxy_Unload", "Proxy don't support \nunloading. Please close \nthe game to exit.", u8"Прокси-режим не поддерживает \nвыгрузку. Пожалуйста, \nзакройте игру, чтобы выйти.", u8"代理模式不支持卸载。\n请关闭游戏以退出。");
	...
	ADD_STR("Menu_DownloadOurChineseFont", "Download Our Chinese Font", u8"Скачать наш китайский шрифт", u8"下载我们的中文字体");
	ADD_STR("Menu_DiscordRPC", "Enable Discord RPC", u8"Включить Discord RPC", u8"启用 Discord 状态显示");
	ADD_STR("Menu_RPCShowName", "Show player name in RPC", u8"Показывать имя игрока в RPC", u8"在 Discord 状态中显示玩家名");
	...
	ADD_STR("FavoriteRoom", "Favorite Room", u8"Любимая комната", u8"鬼魂房间");
    ADD_STR("Location", "Location", u8"Текущая локация", u8"鬼魂当前位置");
	ADD_STR("MimicType", "Mimic Type", u8"Тип мимика", u8"拟魂模仿类型");
	ADD_STR("BansheeTarget", "Banshee Target", u8"Цель банши", u8"女妖锁定目标");
    ...
    ADD_STR("PlayerSanity", "Sanity", u8"Рассудок", u8"理智");
    ADD_STR("PlayerRoom", "Current Room", u8"Текущая комната", u8"当前房间");
    ...
    ADD_STR("PlayerHacker", "Is Hacker", u8"Читер", u8"作弊者");
    ADD_STR("AverageSanity", "Average Sanity", u8"Средний рассудок", u8"平均理智");
    ...
    // FuseBox Modifier
    ADD_STR("EnableFuseBoxModifier", "Enable fusebox modifier", u8"Включить модификатор электрощитка", u8"启用电闸修改器");
    ADD_STR("AutoOnFuse", "Auto On Fuse", u8"Автоматическое включение электрощитка", u8"自动打开电闸");
    ADD_STR("FuseBoxOnByCheat", "FuseBox on by cheat.", u8"Электрощиток включён с помощью чита.", u8"已通过修改器打开电闸");

    // Headers
    ADD_STR("ActivityMonitor_Header", "Activity Monitor", u8"Монитор активности", u8"活动监视器");
    ADD_STR("AlwaysBloodMoon_Header", "Always BloodMoon", u8"Всегда кровавая луна", u8"始终血月");
    ADD_STR("AntiKick_Header", "AntiKick", u8"Антикик", u8"防踢出");
    ...
    ADD_STR("CustomLookLimits_Header", "Custom Look Limits", u8"Пользовательские ограничения обзора", u8"自定义视角限制");
    ...
    ADD_STR("EvidenceESP_Header", "Evidence ESP", u8"ESP доказательств", u8"证据高亮");
    ADD_STR("ExitVanSolo(host)_Header", "Exit Van Solo (host)", u8"Выход из фургона в одиночку (хост)", u8"单人离开（房主）");
    ...
    ADD_STR("JackalopeESP_Header", "Jackalope ESP", u8"ESP на шакалоп", u8"鹿角兔高亮");
    ...
    ADD_STR("NoClip_Header", "NoClip", u8"Ноуклип", u8"穿墙模式");
    ADD_STR("NoEndGame_Header", "No End Game", u8"Отсутствие конца игры", u8"禁止自动结束");
    ADD_STR("NotifyInfo_Header", "Notify Info", u8"Информация уведомлений", u8"游戏通知");
    ADD_STR("PhotoModifier_Header", "Photo Modifier", u8"Модификатор фотографий", u8"照片修改器");
    ADD_STR("Pickup_Header", "Pickup", u8"Поднять предмет", u8"拾取修改");
    ADD_STR("PlayerESP_Header", "Player ESP", u8"ESP на игроков", u8"玩家高亮");
    ...
    ADD_STR("SaltModifier(ONLYYOU)_Header", "Salt Modifier (ONLY YOU)", u8"Модификатор соли (ТОЛЬКО ВЫ)", u8"盐修改器（仅限自己）");
    ...
    ADD_STR("SpiritBoxAlwaysAnswer_Header", "Spirit Box Always Answer", u8"Spirit Box Always Answer", u8"灵盒必定回应");
    ADD_STR("Teleport_Header", "Teleport", u8"Телепортация", u8"传送");
    ...
    ADD_STR("Watermark_Header", "Watermark", u8"Водяной знак", u8"水印");
