...
void Fullbright::FullbrightMain() {
    ...
    if (InGame::FBGO) {
        ...
        if (!SDK::GameObject_get_activeSelf(InGame::FBGO, nullptr)) {
            SDK::GameObject_SetActive(InGame::FBGO, true, nullptr);
        }

        if (!InGame::FBGO) return;

        auto localPlayer = Utils::GetLocalPlayer();
        if (!localPlayer) return;

        auto lightTransform = SDK::GameObject_get_transform(InGame::FBGO, nullptr);
    ...
    SDK::GameObject* gameObject = (SDK::GameObject*)CreateIl2CppObject("UnityEngine.CoreModule", "UnityEngine", "GameObject");
