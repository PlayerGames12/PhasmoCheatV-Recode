...
// базовые структуры (упрощённо, по твоему описанию)
...
extern "C" {
    ...
    // функции il2cpp (будут разрешаться динамически)
    ...
    typedef Il2CppClass* (*t_il2cpp_class_get)(Il2CppImage*, const char* namespaze, const char* name); // fallback
    ...
    // новые функции для работы с полями и классами
    ...
    Il2CppObject* CreateIl2CppObject(const char* assemblyName, const char* namespaze, const char* name); // удобная функция
    Il2CppString* il2cpp_string_new_wrap(const char* str);

    // новые хелперы для работы с полями и классами
